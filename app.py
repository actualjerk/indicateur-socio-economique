...
# Les clés "min"/"max" sont les bornes P5/P95 (normalisation).
# Les clés "min_reel"/"max_reel" sont les bornes réelles (limites de saisie).
# ─────────────────────────────────────────────
...
def normaliser(valeur, vmin, vmax, sens="positif"):
    """Normalise sur [0,1] en utilisant les bornes P5/P95 ; plafonne à 0 ou 1 si hors bornes."""
    if vmax == vmin:
        return 0.0
...
def tableau_variables():
    """Tableau synthétique : affiche les bornes P5/P95, sans les communes."""
    lignes = []
    for nom_dimension, contenu in DIMENSIONS.items():
        for nom_variable, infos in contenu["variables"].items():
            est_documente = infos.get("min_reel") != infos.get("min") or infos.get("max_reel") != infos.get("max")
            lignes.append({
                "Dimension": nom_dimension,
                "Variable": nom_variable,
                "P5": infos["min"],
...
with st.expander("ℹ️ Comment fonctionne cet outil ?", expanded=False):
    ...
    st.markdown('<div class="info-box">🧩 <b>Bornes P5 / P95</b> — La normalisation s\'appuie sur les percentiles 5 et 95 calculés sur les communes d\'Île-de-France. Les valeurs hors de ces bornes sont plafonnées à 0 ou 100. La saisie accepte les vraies valeurs extrêmes observées.</div>', unsafe_allow_html=True)
...
for tab, nom_dimension in zip(tabs, dimensions_choisies):
    with tab:
        ...
        for nom_variable, infos in contenu_dimension["variables"].items():
            ...
            with col_info:
                # Bornes réelles affichées seulement si différentes des bornes P5/P95
                has_reel = (min_reel != p5 or max_reel != p95)
                reel_html = (
                    f'  <span class="borne-arrow" style="margin-left:6px;">|</span>'
                    f'  <span class="borne-block borne-reel-block">'
                    f'    <span class="borne-reel-label">Min réel</span>'
                    f'    <span class="borne-reel-val">{min_reel}{unite_label}</span>'
                    f'  </span>'
                    f'  <span class="borne-arrow">→</span>'
                    f'  <span class="borne-block borne-reel-block">'
                    f'    <span class="borne-reel-label">Max réel</span>'
                    f'    <span class="borne-reel-val">{max_reel}{unite_label}</span>'
                    f'  </span>'
                ) if has_reel else ""
                ...
                st.markdown(
                    ...
                    f'    <span class="borne-label">P5</span>'
                    f'    <span class="borne-val">{p5}{unite_label}</span>'
                    f'  </span>'
                    f'  <span class="borne-arrow">→</span>'
                    f'  <span class="borne-block borne-p95-block">'
                    f'    <span class="borne-label">P95</span>'
                    f'    <span class="borne-val">{p95}{unite_label}</span>'
                    f'  </span>'
                    f'{reel_html}'
                    f'</div>',
                    unsafe_allow_html=True
            ...
            if actif:
                ...
                with col_val:
                    # Détermine le step selon si les bornes réelles sont flottantes
                    ...
                    # Alerte si valeur hors bornes P5/P95
                    if valeur_saisie < p5 or valeur_saisie > p95:
                        st.markdown(
                            f'<div class="alerte-extreme">⚠️ Valeur extrême — en dehors des bornes par percentiles (P5 = {p5}{unite_label}, P95 = {p95}{unite_label})</div>',
                            unsafe_allow_html=True
                        )
...
with st.expander("🔍 Détail du calcul par variable", expanded=False):
    st.caption("Ce tableau montre les variables retenues, leurs bornes P5/P95, poids et score calculé.")
