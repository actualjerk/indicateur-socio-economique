    ...
    "Emploi": {
        ...
        "variables": {
            ...
            "Part des salariés en emploi non stable parmi les salariés": {
                "min": 6.64, "max": 16.69, "valeur": 10.6, "unite": "%",
                "sens": "negatif",
                "source": "INSEE, Recensement de la population 2021, Caractéristiques de l'emploi, base communale, communes d'Île-de-France",
                "commune_min": "Bazoches-sur-Guyonne (78050)", "commune_max": "Rebais (77385)",
                "min_reel": 0.0, "max_reel": 28.37,
                "commune_min_reelle": "3 communes, dont Boisdon (77036)",
                "commune_max_reelle": "Nanteau-sur-Lunain (77329)"
            },
            "Taux d'activité des 15-64 ans": {
                "min": 71.10, "max": 83.52, "valeur": 78.0, "unite": "%",
                "sens": "positif",
                "source": "INSEE, Recensement de la population 2021, base Emploi-Population active, onglet COM_2021, communes d'Île-de-France",
                "commune_min": "Champagne-sur-Seine (77079) et Orsay (91471)", "commune_max": "Guiry-en-Vexin (95295) et Moisenay (77295)",
                "min_reel": 42.48, "max_reel": 88.84,
                "commune_min_reelle": "Fleury-Mérogis (91235)",
                "commune_max_reelle": "Dhuisy (77157)"
            },
        },
    },
    "Santé": {
    ...
    for nom_dimension, contenu in DIMENSIONS.items():
        for nom_variable, infos in contenu["variables"].items():
            ...
            ligne = {
                ...
                "Variable": nom_variable,
                "Min (P5)": infos["min"],
                "Commune borne min (P5)": infos.get("commune_min", ""),
                "Max (P95)": infos["max"],
                "Commune borne max (P95)": infos.get("commune_max", ""),
                "Unité": infos.get("unite", ""),
                "Sens": "✅ positif" if infos.get("sens") == "positif" else "🔻 négatif",
                "Documenté": "✅" if est_documente else "⏳",
                "Source": source_courte
            }
            if "min_reel" in infos:
                ligne["Min réel"] = infos["min_reel"]
                ligne["Commune min réelle"] = infos.get("commune_min_reelle", "")
                ligne["Max réel"] = infos["max_reel"]
                ligne["Commune max réelle"] = infos.get("commune_max_reelle", "")
            lignes.append(ligne)
    ...
    with tab:
        ...
        for nom_variable, infos in contenu_dimension["variables"].items():
            ...
            with col_info:
                ...
                # Label dynamique selon si c'est un percentile ou une borne brute
                has_percentile = "min_reel" in infos
                label_min = "P5" if has_percentile else "Min"
                label_max = "P95" if has_percentile else "Max"

                # Bornes réelles optionnelles
                bornes_reelles_html = ""
                if has_percentile:
                    min_reel = infos.get("min_reel", "")
                    max_reel = infos.get("max_reel", "")
                    commune_min_reelle = infos.get("commune_min_reelle", "")
                    commune_max_reelle = infos.get("commune_max_reelle", "")
                    bornes_reelles_html = (
                        f'<div style="margin-top:4px; font-size:0.75rem; color:#94a3b8;">'
                        f'Étendue réelle : <b>{min_reel}{unite_label}</b> ({commune_min_reelle}) '
                        f'→ <b>{max_reel}{unite_label}</b> ({commune_max_reelle})'
                        f'</div>'
                    )
