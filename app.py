...
def explication_borne(nom_variable, p5, p95, unite, min_reel, max_reel, commune_min, commune_max):
    ...
    texte_p5 = f"Environ 5 % des communes d'Île-de-France ont une valeur inférieure à {p5_fmt}{u} pour cette variable."
    texte_p95 = f"Environ 5 % des communes d'Île-de-France ont une valeur supérieure à {p95_fmt}{u} pour cette variable."
...
with st.expander("🔍 Détail du calcul par variable", expanded=False):
    ...
    for col in ["Valeur saisie", "P5", "P95", "Poids variable", "Score normalisé 0-100"]:
        df_affichage[col] = df_affichage[col].apply(
            lambda v: int(v) if isinstance(v, float) and v == int(v) else v
        )
